...
public class AllureReporter implements StoryReporter {
    ...
    public static ThreadLocal<Boolean> isStepFailed = new ThreadLocal<>();
    public static ThreadLocal<Throwable> stepFailedCause = new ThreadLocal<>();
    ...
    public void beforeScenario(String scenarioTitle) {
        isStepFailed.set(false);
        stepFailedCause.set(new Throwable());
...
//		Thread.currentThread().setName(scenarioTitle);
    ...
    public void successful(String step) {
        if(null!=isStepFailed.get() && true == isStepFailed.get()){
            continueFailedStep(step, stepFailedCause.get());
        } else {
...
//    private static Throwable stepFailed = null;
    ...
    public void continueFailedStep(String step, Throwable cause) {
        ...
        allure.fire(new StepFailureEvent().withThrowable(trueCause));
        allure.fire(new StepFinishedEvent());
        allure.fire(new TestCaseFailureEvent().withThrowable(trueCause));
    }
...
//	}
	...
	private static void generateProperties(String directory){
        ...
        environmentToShow = populateProperty("Threads","bdd.thread", environmentToShow);
    ...
    public static void copyTargetFolderToReportBackupFolder(){
        try {
            if((!ConfigLoader.config().getString("JIRA.REPORTS_DIR").isEmpty()) && (!ConfigLoader.config().getString("JIRA.REPORTS_DIR").equalsIgnoreCase("NA")) ){
                File dir = new File(ConfigLoader.config().getString("JIRA.REPORTS_DIR")+"/"+ ConfigLoader.config().getString("JIRA.BUILD_NUMBER"));
